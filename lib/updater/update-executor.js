...
export class UpdateExecutor {
  ...
  async executeStep(step, { dryRun = true } = {}) {
    ...
    if (this._type === "git" && step === "install") {
      try {
        ...
        if (status?.trim()) {
          return {
            ...
            output: "Refusing to update a dirty Git checkout. Commit or stash your work, then retry.",
            nextStep: null,
        ...
        return {
          ...
          dryRun: false,
          commands: plan.commands,
          output: `Unable to verify the Git working tree: ${err.message}`,
          nextStep: null,
          restartRequired: false
        };
      }
  ...
  _fetch_npm_local()  { return { commands: [{ cmd: "npm", args: ["view", "weasley-deepmind", "versions", "--json"] }] }; }
  _fetch_npm_global() { return this._fetch_npm_local(); }
  _fetch_docker()     { return { commands: [], message: "Check GHCR for the latest Weasley DeepMind image tags" }; }
  _fetch_unknown()    { return { commands: [], message: "Visit https://github.com/potatohoney-p/weasley-deepmind/tags" }; }
  ...
  _install_git() {
    return {
      commands: [
        ...
        { cmd: "npm", args: ["ci", "--omit=dev"], opts: { cwd: this._root } }
      ],
      restartRequired: true
    };
  }
  _install_npm_local()  { return { commands: [{ cmd: "npm", args: ["update", "weasley-deepmind"] }], restartRequired: true }; }
  _install_npm_global() { return { commands: [{ cmd: "npm", args: ["install", "-g", `weasley-deepmind@${this._semver()}`] }], restartRequired: true }; }
  _install_docker()     { return { commands: [], message: "Update outside the container:\n  docker pull ghcr.io/potatohoney-p/weasley-deepmind:latest\n  docker compose up -d", restartRequired: false }; }
