...
export const rememberDefinition = {
  ...
  description : "자기완결적 사실 하나를 파편으로 저장. 저장된 content는 몇 달 뒤 전혀 다른 세션에서 " +
                 ...
                 "응답 필드 (v2.8.0+, 기본 비활성):\n" +
                 "- validation_warnings: WEASLEY_DEEPMIND_SYMBOLIC_POLICY_RULES=true 시 PolicyRules 위반 " +
  ...
  inputSchema: {
    ...
    properties: {
      content: {
        ...
        examples   : [
          "weasley-deepmind v2.12.0 배포 완료 (2026-04-20). migration-031 적용, fragment_links.weight 컬럼 추가.",
          "사용자 인증 API 응답 시간을 500ms에서 120ms로 개선 (ORM N+1 쿼리를 JOIN 단일 쿼리로 변경)",
          "로그인 실패 원인: JWT 만료 검증의 timezone 불일치 → 서버·클라이언트 모두 UTC로 통일하여 해결",
          "사용자 선호: 커밋 메시지에 이모지 사용 금지"
        ]
      ...
      topic: {
        ...
        description: "파편의 주제 식별자. recall 시 topic 필터로 활용되며, " +
                     ...
                     "프로젝트명 또는 도메인 단위로 지정 권장 (예: weasley-deepmind, authentication, deployment).",
        examples   : [
          "weasley-deepmind",
          "authentication",
          "deployment",
      ...
      contextSummary: {
        ...
        examples   : ["weasley-deepmind v2.11.0 배포 중 migration-030 적용 직후 발생한 오류 분석 맥락"]
      },
      sessionId: {
        type       : "string",
      ...
      workspace: {
        type       : "string",
        description: "워크스페이스 이름 (예: 'weasley-deepmind', 'personal', 'client-acme'). " +
                     "미지정 시 키의 default_workspace 적용. " +
                     "전역 기억(모든 워크스페이스에서 조회)으로 저장하려면 지정하지 않고 키에 default_workspace도 없어야 함.",
        examples   : ["weasley-deepmind", "personal", "client-acme"]
      },
      ...
      goal: {
        ...
        examples   : ["weasley-deepmind CLI remote 연결 기능 구현 완료"]
      },
      outcome: {
        type       : "string",
        ...
        items      : {
          ...
          properties: {
            content   : {
              ...
              description: "기억할 내용 (1~3문장, 300자 이내 권장)",
              examples   : [
                "weasley-deepmind v2.12.0 배포 완료 (2026-04-20). migration-031 적용.",
            ...
            topic     : {
              type       : "string",
              description: "주제",
              examples   : ["weasley-deepmind", "deployment", "authentication"]
            },
            type      : {
              type: "string",
              enum: ["fact", "decision", "error", "preference", "procedure", "relation", "episode"],
            ...
            keywords       : {
              ...
              items      : { type: "string" },
              description: "검색용 키워드",
              examples   : [["weasley-deepmind", "cli"], ["api", "ratelimit"]]
            },
            workspace      : {
              type       : "string",
              description: "워크스페이스 이름 (미지정 시 키의 default_workspace 적용)",
              examples   : ["weasley-deepmind", "personal"]
            },
            idempotencyKey : {
              type       : "string",
              maxLength  : 128,
...
export const recallDefinition = {
  ...
  description : "파편 기억 검색. 키워드, 주제, 유형, 자연어 쿼리로 관련 기억을 회상한다. remember/reflect로 저장한 기억을 불러오는 retrieval 짝이다. " +
                 ...
                 "응답 필드 (v2.8.0+, WEASLEY_DEEPMIND_SYMBOLIC_EXPLAIN=true 시): 각 파편에 " +
    ...
    properties: {
      keywords: {
        ...
        examples   : [
          ...
          ["weasley-deepmind", "migration"]
        ]
      },
      topic: {
        type       : "string",
        description: "주제 필터. remember 시 지정한 topic과 정확히 일치하는 파편만 반환.",
        examples   : ["weasley-deepmind", "authentication", "deployment"]
      },
      type: {
        type       : "string",
        enum       : ["fact", "decision", "error", "preference", "procedure", "relation", "episode"],
      ...
      contextText: {
        ...
        examples   : [
          "weasley-deepmind CLI remote 연결 기능을 구현 중. --remote 플래그와 인증 헤더 처리가 핵심.",
  ...
  inputSchema: {
    ...
    properties: {
      ...
      topic: {
        ...
        examples   : ["nginx-ssl-error", "weasley-deepmind"]
      },
      force: {
        type       : "boolean",
  ...
  inputSchema: {
    ...
    properties: {
      ...
      content: {
        ...
        examples   : [
          "weasley-deepmind SearchParamAdaptor: key_id integer → text 변경 완료 (migration-030). UUID keyId 정규화 포함.",
      ...
      topic: {
        type       : "string",
        description: "새 주제",
        examples   : ["weasley-deepmind", "authentication"]
      },
      keywords: {
        type       : "array",
        items      : { type: "string" },
        description: "새 키워드 목록",
        examples   : [["weasley-deepmind", "migration", "key_id"], ["authentication", "port", "security"]]
      },
      type: {
        type       : "string",
...
export const getSkillGuideDefinition = {
  ...
  description: "Weasley DeepMind 최적 활용 가이드를 반환한다. " +
  ...
  inputSchema: {
    ...
    properties: {
      ...
      entity   : {
        ...
        examples   : ["weasley-deepmind", "nginx"]
      },
      timeRange: {
        type      : "object",
        properties: {
