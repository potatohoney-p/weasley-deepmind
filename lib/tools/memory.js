...
function buildRecallResponse(result, { args, linkedMap, contradictionPending = false }) {
  ...
  const fragments = result.fragments.map(f => {
    const projected = {
      ...
      /** Phase 2 Explainability: WEASLEY_DEEPMIND_SYMBOLIC_EXPLAIN=true 시 FragmentSearch가 주입 */
...
export async function tool_reflect(args) {
  ...
  return withAudit("reflect", args, async (a, sessionId) => {
    ...
    const { _link_suggestions, _searchEventId, _weasley_deepmind_hint, _suggestion, ...restResult } = result;
    return {
      response: {
        success: true,
        ...restResult,
