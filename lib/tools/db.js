 ...
 * - application_name='weasley-deepmind:batch': pg_stat_activity 분리 모니터링
 *
  ...
  if (!poolBatch) {
    ...
    const batchConfig = batchUrl
      ? {
          ...
          idleTimeoutMillis      : DB_IDLE_TIMEOUT_MS,
          connectionTimeoutMillis: DB_CONN_TIMEOUT_MS,
          application_name       : "weasley-deepmind:batch"
        }
      : {
          host                   : DB_HOST,
          port                   : DB_PORT,
...
export async function queryWithAgentVector(agentId, sql, params = [], opts = {}) {
  ...
  try {
     ...
     */
    if (opts.forceVectorIndex && process.env.WEASLEY_DEEPMIND_VECTOR_FORCE_INDEX !== "off") {
