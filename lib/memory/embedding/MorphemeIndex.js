 ...
 *
 * 토크나이저 경로는 WEASLEY_DEEPMIND_MORPHEME_TOKENIZER 환경변수로 선택한다.
...
export class MorphemeIndex {
   ...
   * 텍스트를 형태소 목록으로 분리
   *
   * WEASLEY_DEEPMIND_MORPHEME_TOKENIZER=local(기본값) 시 MorphemeTokenizer 로컬 분석기 경유.
   * WEASLEY_DEEPMIND_MORPHEME_TOKENIZER=llm 시 레거시 Gemini CLI 경로(_tokenizeViaLLM) 사용.
  ...
  /** @private 레거시 LLM 경로 (WEASLEY_DEEPMIND_MORPHEME_TOKENIZER=llm 시에만 사용) */
