...
export class MemoryRememberer {
  ...
  async remember(params) {
     ...
     * 할당량 초과 검사.
     *
     * WEASLEY_DEEPMIND_REMEMBER_ATOMIC=true: BEGIN → api_keys FOR UPDATE(quota 재검증) →
     *   FragmentWriter.insert(client) → COMMIT 단일 트랜잭션. TOCTOU 완전 차단.
     *   BatchRememberProcessor Phase B와 동일한 SELECT 조건·잠금 범위 사용.
     *
     * WEASLEY_DEEPMIND_REMEMBER_ATOMIC=false(기본): QuotaChecker.check()를 선제 검사로만 사용.
     *   동시 요청이 드문 환경에서 기존 성능·동작을 그대로 보존한다.
     */
    const atomicRemember = process.env.WEASLEY_DEEPMIND_REMEMBER_ATOMIC === "true";
   ...
   * WEASLEY_DEEPMIND_REMEMBER_ATOMIC=true이고 keyId가 존재할 때만 호출된다.
   *
