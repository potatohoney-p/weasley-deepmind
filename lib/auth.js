 ...
 *
 * @param {string}  accessKey   - WEASLEY_DEEPMIND_ACCESS_KEY 값
 * @param {boolean} authDisabled - WEASLEY_DEEPMIND_AUTH_DISABLED=true 여부
 ...
 * 인증 검증
 * 우선순위:
 * 1. weasley-deepmind-access-key 헤더
...
export async function validateAuthentication(req, msg) {
  if (!ACCESS_KEY) {
    ...
    if (decision.valid) {
      return { ...decision, groupKeyIds: null };
    }

    recordAuthDenied(decision.reason || "access_key_required");
    return {
      valid: false,
      error: "Server authentication is not configured. Set WEASLEY_DEEPMIND_ACCESS_KEY or explicitly opt out with WEASLEY_DEEPMIND_AUTH_DISABLED=true for local development."
    };
  }

  /** 1. Weasley DeepMind 헤더 체크 */
  const legacyKey          = req.headers["weasley-deepmind-access-key"];
