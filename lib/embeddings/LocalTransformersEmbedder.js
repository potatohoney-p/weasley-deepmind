    ...
    if (!this._initPromise) {
      this._initPromise = (async () => {
        ...
        try {
          ...
          throw new Error(
            "Local transformer embeddings are optional. Install @huggingface/transformers and pin a patched sharp release as documented in docs/embedding-local.md.",
            { cause: error }
          );
        }

        console.info(`[LocalEmbedder] loading model ${this.modelId} (dtype=q8)`);
        this._pipeline = await pipeline("feature-extraction", this.modelId, { dtype: "q8" });
