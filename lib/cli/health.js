...
export const usage = [
  "Usage: weasley-deepmind health [options]",
  "",
  ...
  "",
  "Examples:",
  "  weasley-deepmind health",
  "  weasley-deepmind health --json",
  "  weasley-deepmind health --format csv",
].join("\n");
...
async function checkMigrations(pool) {
  try {
    ...
    const dir  = path.join(import.meta.dirname, '..', 'memory', 'migrations');
...
export default async function health(opts) {
  ...
  try {
    ...
    console.log("Weasley DeepMind Health Check");
