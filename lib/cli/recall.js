...
export const usage = [
  "Usage: weasley-deepmind recall <query> [options]",
  "",
  ...
  "",
  "Examples:",
  "  weasley-deepmind recall nginx ssl",
  "  weasley-deepmind recall auth --topic backend --limit 5",
  "  weasley-deepmind recall deploy --format csv",
  "  weasley-deepmind recall nginx --remote https://deepmind.example.com/mcp --key wdm_xxx",
].join("\n");

export default async function recall(args) {
  const query = args._.join(" ");
  if (!query) {
    console.error("Usage: weasley_deepmind recall <query> [--topic x] [--limit n] [--time-range from,to] [--json]");
    process.exit(1);
  }
