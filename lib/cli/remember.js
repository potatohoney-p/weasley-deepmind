...
export const usage = [
  "Usage: weasley-deepmind remember <content> --topic <name> [options]",
  "       weasley-deepmind remember --stdin --topic <name> [options]",
  "       echo 'content' | weasley-deepmind remember --topic <name> [options]",
  "",
  ...
  "  --remote <URL>        MCP 원격 서버 URL (env: WEASLEY_DEEPMIND_CLI_REMOTE)",
  "  --key <KEY>           API 키 Bearer 토큰 (env: WEASLEY_DEEPMIND_CLI_KEY)",
  ...
  "",
  "Examples:",
  "  weasley-deepmind remember 'Redis port is 6380' --topic infra --type fact",
  "  weasley-deepmind remember 'Use bcrypt rounds=12' --topic security --type decision --importance 0.8",
  "  cat note.txt | weasley-deepmind remember --topic docs --type procedure",
  "  weasley-deepmind remember --stdin --topic ops < fix.txt",
  "  weasley-deepmind remember 'nginx restart fixed' --topic ops --remote https://deepmind.example.com/mcp --key wdm_xxx",
].join("\n");

export default async function remember(args) {
  const remoteUrl     = args.remote || process.env.WEASLEY_DEEPMIND_CLI_REMOTE;
  const remoteKey     = args.key    || process.env.WEASLEY_DEEPMIND_CLI_KEY;
  ...
  if (!content || !content.trim()) {
    console.error("Usage: weasley_deepmind remember <content> --topic x [--type fact] [--importance 0.7] [--json]");
    process.exit(1);
  }
  ...
  if (remoteUrl && !remoteKey) {
    console.error("--remote 사용 시 --key <API_KEY> 또는 WEASLEY_DEEPMIND_CLI_KEY 환경변수가 필요합니다.");
    process.exit(1);
  }
