...
async function cmdShow(args, remoteUrl, remoteKey, timeoutMs) {
  ...
  if (!sessionId) {
    console.error("Usage: weasley-deepmind session show <sessionId>");
    process.exit(1);
  }
...
async function cmdDelete(args, remoteUrl, remoteKey, timeoutMs) {
  const sessionId = args._[1];
  if (!sessionId) {
    console.error("Usage: weasley-deepmind session delete <sessionId>");
    process.exit(1);
  }
...
async function cmdRotate(args, remoteUrl, remoteKey, timeoutMs) {
  const sessionId = args._[1];
  if (!sessionId) {
    console.error("Usage: weasley-deepmind session rotate <sessionId> [--reason X]");
    process.exit(1);
  }
...
export const usage = [
  "Usage: weasley-deepmind session <subcommand> [options]",
  "",
  ...
  "",
  "Examples:",
  "  weasley-deepmind session list",
  "  weasley-deepmind session list --limit 20 --format json",
  "  weasley-deepmind session show abc123-...",
  "  weasley-deepmind session delete abc123-...",
  "  weasley-deepmind session rotate abc123-... --reason suspected_leak",
  "  weasley-deepmind session rotate abc123-... --remote http://localhost:57332 --key wdm_xxx",
  "  weasley-deepmind session list --remote http://localhost:57332 --key wdm_xxx",
].join("\n");
...
export default async function session(args) {
  ...
  if (remoteUrl && !remoteKey) {
    console.error("--remote 사용 시 --key <KEY> 또는 WEASLEY_DEEPMIND_CLI_KEY 환경변수가 필요합니다.");
    process.exit(1);
  }
  ...
  switch (sub) {
    ...
    default:
      ...
      console.error('Available: list, show, delete, rotate. Run "weasley-deepmind session --help" for usage.');
      process.exit(1);
  }
}
