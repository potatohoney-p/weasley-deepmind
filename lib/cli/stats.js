...
export const usage = [
  "Usage: weasley-deepmind stats [options]",
  "",
  ...
  "",
  "Examples:",
  "  weasley-deepmind stats",
  "  weasley-deepmind stats --format csv",
  "  weasley-deepmind stats --json",
  "  weasley-deepmind stats --remote https://deepmind.example.com/mcp --key wdm_xxx",
].join("\n");

const SCHEMA = 'agent_memory';

export default async function stats(opts) {
  const remoteUrl = opts.remote || process.env.WEASLEY_DEEPMIND_CLI_REMOTE;
  const remoteKey = opts.key    || process.env.WEASLEY_DEEPMIND_CLI_KEY;
  ...
  if (remoteUrl) {
    ...
    try {
      ...
      console.log("Weasley DeepMind Statistics (remote)");
  ...
  try {
    ...
    console.log("Weasley DeepMind Statistics");
    console.log(renderTable(summaryRows, ["key", "value"]));
