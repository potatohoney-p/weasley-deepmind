// lib/cli/update.js
export const usage = [
  "Usage: weasley-deepmind update [options]",
  "",
  ...
  "",
  "Examples:",
  "  weasley-deepmind update",
  "  weasley-deepmind update --execute",
  "  weasley-deepmind update --execute --redetect",
].join("\n");
...
export async function run(args) {
  ...
  for (const step of ["fetch", "install", "migrate"]) {
    ...
    if (!await confirm(`\n${label}?`)) {
      ...
      if (step === "migrate") console.error("Run manually: weasley-deepmind migrate");
      process.exit(0);
    }
