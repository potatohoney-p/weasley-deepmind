...
export const REDACT_PATTERNS = [
  ...
  /** Cookie 헤더의 wdm_session 값 — wdm_ 키 패턴보다 먼저 처리 */
  { pattern: /(wdm_session\s*=\s*)[^;\s"]+/g,             replacement: "$1****"   },
  /** wdm_ API 키 패턴 (wdm_로 시작하되 session= 뒤 아닌 경우) */
  { pattern: /\bwdm_(?!session\s*=)[A-Za-z0-9_-]+/g,     replacement: "wdm_****"},
  /** OAuth code 파라미터 */
