...
export async function handleInitialize(params) {
  ...
  try {
    ...
    const aiInstructions     = `# Weasley DeepMind Server
...
2. 플랫폼에 기억 도구 관련 설정(CLAUDE.md, 훅, .cursorrules 등)이 없는 경우:
   - get_skill_guide 도구를 호출하여 Weasley DeepMind 최적 활용 가이드를 로드한다.
...
| recall 응답에 _meta.hints[0] 포함 | hint.trigger 도구 고려 | hint.suggestion 참조 (v3.1.0에서 top-level _weasley_deepmind_hint mirror 제거 완료. _meta.hints[0]만 사용) |
| context 응답에 _meta.hints[0] 포함 | hint.trigger 도구 고려 | hint.suggestion 참조 (v3.1.0에서 top-level _weasley_deepmind_hint mirror 제거 완료. _meta.hints[0]만 사용) |
...
- 프로젝트 작업인 경우: keywords에 프로젝트명을 반드시 포함 (예: "weasley-deepmind", "docs-mcp")
...
- recall: 기억 검색 (키워드, 주제, 시맨틱 검색)
  ...
  - 응답 파편에 explanations[].code가 포함되면 해당 파편이 검색된 이유를 설명한다 (v2.8.0+, WEASLEY_DEEPMIND_SYMBOLIC_EXPLAIN=true 필요).
- forget: 기억 삭제
- link: 파편 간 관계 설정
...
- get_skill_guide: Weasley DeepMind 최적 활용 스킬 가이드 (전체 또는 섹션별)
...
- 사실 검색은 Weasley DeepMind가 빠르고 정확하다. 전후관계가 중요한 기억은 episode 유형이나 메인 메모리 시스템과 병행을 권장한다.
...
지원 버전: ${SUPPORTED_PROTOCOL_VERSIONS.join(", ")}`;
    ...
    const result = {
      ...
      serverInfo     : {
        name       : "weasley-deepmind-server",
        version    : PKG_VERSION,
        description: `Weasley DeepMind - Fragment-Based Memory Server (도구 ${toolCount}개)
...
- batchPool 분리 (getBatchPool, application_name='weasley-deepmind:batch', BATCH_DATABASE_URL 옵션)
