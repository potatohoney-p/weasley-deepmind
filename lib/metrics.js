...
/** 기본 메트릭 활성화 (CPU, 메모리 등) — 단위 테스트 환경에서 WEASLEY_DEEPMIND_METRICS_DEFAULT=off 로 비활성화 */
if (process.env.WEASLEY_DEEPMIND_METRICS_DEFAULT !== "off") {
...
export const authDeniedTotal = new prometheus.Counter({
  name      : "weasley_deepmind_auth_denied_total",
...
export const corsDeniedTotal = new prometheus.Counter({
  name      : "weasley_deepmind_cors_denied_total",
...
export const rbacDeniedTotal = new prometheus.Counter({
  name      : "weasley_deepmind_rbac_denied_total",
...
export const tenantIsolationBlockedTotal = new prometheus.Counter({
  name      : "weasley_deepmind_tenant_isolation_blocked_total",
 ...
 *
 * BatchRememberProcessor 전용 분리 풀(application_name='weasley-deepmind:batch')의
 * active/idle 커넥션 수를 Prometheus에 노출한다.
 * pg_stat_activity 필터 조건: application_name = 'weasley-deepmind:batch'
 */
