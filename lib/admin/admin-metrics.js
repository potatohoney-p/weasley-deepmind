...
let _samplingEnabled = process.env.WEASLEY_DEEPMIND_ADMIN_METRICS_SAMPLING !== "off";
...
export async function buildMetricsSummary({
  ...
  if (include.includes("cards")) {
    ...
    /** Counter rates */
    const authDenied      = sumCounterValues(map.get("weasley_deepmind_auth_denied_total"));
    const rbacDenied      = sumCounterValues(map.get("weasley_deepmind_rbac_denied_total"));
    const tenantBlocked   = sumCounterValues(map.get("weasley_deepmind_tenant_isolation_blocked_total"));
    const toolErrors      = sumCounterValues(map.get("mcp_errors_total"), { type: "tool" });
    const symbolicBlocked = sumCounterValues(map.get("weasley_deepmind_symbolic_gate_blocked_total"));
