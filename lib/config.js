...
export const HOST               = process.env.WEASLEY_DEEPMIND_HOST || process.env.HOST || "127.0.0.1";
...
export const ACCESS_KEY         = process.env.WEASLEY_DEEPMIND_ACCESS_KEY || "";

/**
 * 빈 ACCESS_KEY의 fail-closed 동작을 우회하는 명시적 opt-in 플래그.
 * WEASLEY_DEEPMIND_AUTH_DISABLED=true 로만 활성화 가능.
 * 활성화 시 서버는 모든 요청을 master 권한으로 처리한다 (개발/테스트 전용).
 */
export const AUTH_DISABLED      = process.env.WEASLEY_DEEPMIND_AUTH_DISABLED === "true";
 ...
 */
export const CASE_BACKPROP_ENABLED = process.env.WEASLEY_DEEPMIND_CASE_BACKPROP_ENABLED === "true";
...
export function resolveSplitChainConfig(env = process.env) {
  const primary = env.WEASLEY_DEEPMIND_SPLIT_LLM_PRIMARY;
  const rawFb   = env.WEASLEY_DEEPMIND_SPLIT_LLM_FALLBACKS;
