 ...
 * 우선순위: Authorization Bearer → weasley_deepmind-access-key → initialize.params.accessKey.
...
export function deriveTokenKey(req, msg, authCheck) {
  const raw = (() => {
    ...
    if (req.headers["weasley_deepmind-access-key"]) return req.headers["weasley_deepmind-access-key"];
...
async function _createInitializeSession(req, res, msg) {
   ...
   * 동일 Bearer/WEASLEY_DEEPMIND_ACCESS_KEY/initialize.accessKey 토큰으로 들어오면
...
function _resolveMode(req, msg, dbDefaultMode, _keyId) {
  /** 1. X-Weasley DeepMind-Mode 헤더 (최우선) */
  const headerMode = req.headers["x-weasley_deepmind-mode"] ?? null;
  if (headerMode) {
