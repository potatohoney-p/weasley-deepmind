...
| `onboarding` | 시스템 사용법 안내 | AI가 weasley-deepmind의 도구들을 언제 어떻게 써야 하는지 스스로 학습 |
...
weasley-deepmind 최적 활용 가이드를 반환한다. 기억 도구 사용법, 세션 생명주기, 키워드 규칙, 검색 전략, 경험적 기억 활용법 등 포괄적 스킬 레퍼런스.
...
```bash
# 응답 헤더 확인
curl -si -X POST https://weasley-deepmind.example.com/mcp \
