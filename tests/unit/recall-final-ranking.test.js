...
describe("computeRecallScore", () => {
  ...
  it("lexWeight는 파편별 rerankerScore 유무로 결정 (집합 단위 아님)", () => {
    ...
    const reranked   = { rerankerScore: 0.50, topic: "weasley-deepmind" };
    const unreranked = { topic: "weasley-deepmind", importance: 0.5, similarity: 0,
                         created_at: new Date().toISOString() };
    const ctx = baseCtx({ lexicalQuery: { topic: "weasley-deepmind" } });
  ...
  it("rerankerScore 격차를 lexical 보정이 무조건 뒤집지 않는다", () => {
    ...
    const lowLex = { rerankerScore: 0.70, topic: "weasley-deepmind" };
    const ctx = baseCtx({ lexicalQuery: { topic: "weasley-deepmind" } });
  ...
  it("reranker 비활성 시 직접 topic 일치가 무관 최신 파편보다 상위", () => {
    const now = Date.now();
    const onTopicOld = {
      topic: "weasley-deepmind", importance: 0.5,
    ...
    const ctx = baseCtx({ anchorTime: now, lexicalQuery: { topic: "weasley-deepmind" } });
    assert.ok(
  ...
  it("연결 파편(_source=linked)은 lexical 가중치가 절반", () => {
    const direct = { topic: "weasley-deepmind", importance: 0.3, similarity: 0, created_at: new Date().toISOString() };
  ...
  it("log 스케일: lexRaw 강도 차이가 변별력을 유지", () => {
    ...
    const ctx = baseCtx({ lexicalQuery: { topic: "weasley-deepmind", keywords: ["a","b","c"] } });
    const weak   = { topic: "weasley-deepmind", importance: 0, similarity: 0,
                     created_at: new Date().toISOString() }; // topic exact = 4
    const strong = { ...weak, keywords: ["weasley-deepmind", "a", "b", "c"], content: "a b c" };
  ...
  it("hard tier 없음 — content 1회 매치가 완벽 의미매치를 압도하지 않음", () => {
    ...
    const weakLexical     = { rerankerScore: 0.3, content: "weasley_deepmind 등장" };
    const ctx = baseCtx({ lexicalQuery: { keywords: ["weasley_deepmind"] } });
    assert.ok(
