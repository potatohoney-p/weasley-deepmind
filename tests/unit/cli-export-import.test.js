...
function writeTempJsonl(rows) {
  const file = path.join(os.tmpdir(), `weasley_deepmind-test-${Date.now()}-${Math.random().toString(36).slice(2)}.jsonl`);
...
/** ---- bin/weasley-deepmind.js 등록 확인 ---- */
describe("M6: bin/weasley-deepmind.js COMMANDS 등록", () => {
  it("export와 import가 COMMANDS에 등록됨", async () => {
    const src = fs.readFileSync(
      fileURLToPath(new URL("../../bin/weasley-deepmind.js", import.meta.url)),
      "utf8"
    );
