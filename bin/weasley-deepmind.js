#!/usr/bin/env node
/**
 * Weasley DeepMind CLI entrypoint.
 *
...
const LOCAL_ONLY_COMMANDS = new Set(["serve", "stdio", "migrate", "cleanup", "backfill", "health", "update", "export", "import"]);

/**
 * `weasley-deepmind --help` 출력 텍스트를 stdout으로 송출한다.
 */
function printUsage() {
  const lines = [
    'Usage: weasley-deepmind <command> [options]',
    '',
    'Commands:',
    '  serve                       Start the MCP server',
    '  stdio                       Run as a local stdio MCP server',
    ...
    'Local-only commands: serve, stdio, migrate, cleanup, backfill, health, update, export, import',
  ];
...
async function main() {
  ...
  if (!COMMANDS[cmd]) {
    ...
    console.error('Run "weasley-deepmind --help" for usage.');
    process.exit(1);
  }
  ...
  const remoteUrl = args.remote
    || process.env.WEASLEY_DEEPMIND_CLI_REMOTE
    || process.env.WEASLEY_DEEPMIND_CLI_REMOTE;
  ...
  if (cmd !== "update" && process.env.UPDATE_CHECK_DISABLED !== "true") {
    import("../lib/updater/cache.js").then(async ({ UpdateCache }) => {
      ...
      try {
        ...
        if (r.updateAvailable) process.stderr.write(`\n[weasley-deepmind] v${r.latestVersion} available. Run "weasley-deepmind update" to upgrade.\n`);
